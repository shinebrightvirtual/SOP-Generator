            ...
            <div style={{ fontSize: "12px", color: "#4A4A4A", lineHeight: 1.6, marginTop: "6px" }}>
              {sec.fields.map(field => {
                    ...
                    <div key={field.key}>
                      {val.filter(s => s.what?.trim()).map((s, i) => (
                        <div key={i} style={{ marginBottom: "8px", paddingLeft: "10px", borderLeft: `2px solid ${ac}` }}>
                          ...
                          <div>{s.what}</div>
                          {s.tools && <div style={{ fontSize: "11px", color: colors.textMuted }}>Tools: {s.tools}</div>}
                          {s.time && <div style={{ fontSize: "11px", color: colors.textMuted }}>Time: {s.time}</div>}
                        </div>
                      ))}
                    </div>
                  );
                ...
                if (field.type === "bulletlist" && Array.isArray(val)) {
                  return (
                    <div key={field.key} style={{ marginBottom: "6px" }}>
                      <div style={{ fontSize: "11px", fontWeight: 600, color: colors.textSubLabel, marginBottom: "3px" }}>{field.label}</div>
                      {val.filter(v => v.trim()).map((v, i) => (
                ...
                return (
                  <div key={field.key} style={{ marginBottom: "6px" }}>
                    <span style={{ fontWeight: 600, color: colors.textSecondary }}>{field.label}: </span>
                    <span>{val}</span>
                  </div>
                );
              })}
